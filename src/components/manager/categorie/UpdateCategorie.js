...
const UpdateRecette = (props) => {
  ...
  useEffect(() => {
      ...
      setSliderImages(props.categorie.images);
      setProductName(props.categorie.nom)    ;
      setEditorState(props.categorie.description);
      },[]);
  ...
  const handleCreateProduct = async () => {
    ...
    const reponse = await fetch(
      "http://localhost:5000/api/categorie/update/" + props.categorie._id,
      requestOptions
    );
    const data = await reponse.json();
    if (reponse.status === 200) {
      swal({
        title: "Categorie was updated successfully!",
        text: "",
        icon: "success",
        button: "Ok",
      });
  ...
  return (
    <div className="p-8 relative">
      ...
      <h1 className="text-2xl font-black">Modifier la categorie</h1>
