...
import UpdateIngrediant from "./UpdateIngrediant";
...
const columns = [
  { id: 'ingrediant', label: 'Ingrediants', minWidth: 130 },
  { id: 'nom', label: 'Nom', minWidth: 130 },
  { id: 'description', label: 'Description', minWidth: 130 },
  { id: 'energie', label: 'Energie en calorie ', minWidth: 130 }
];


function createData(ingrediant, nom,description,energie) {
  return { ingrediant, nom,description,energie};
}
...
function Ingrediants() {
  ...
  const getIngrediants=async()=>{


    const url="http://localhost:5000/api/ingrediant/getAll/62939b45762a0af0bce00c28";
    const res=  await  axios.get(url);
    
    setRecettes(res.data.ingrediant);
    console.log(res.data.ingrediant)
  ...
  const deleteIngrediants=async(id)=>{
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const url="http://localhost:5000/api/ingrediant/delete/"+id;
...
//     button=  <button style={{ width:"120px",height:"35px",color:"white",backgroundColor:"#FA5B33",border:"none",borderRadius:"3px"}}>{props.status}</button>;
    ...
    let modelShow;
    if(statModel==="add"){
        modelShow=<AddIngrediant handleCloseModel={handleCloseModel}/>
    }
    if(statModel==="update"){
        modelShow=<UpdateIngrediant ingrediant={dataPassToModelUpdat} handleCloseModel={handleCloseModel}/>
    }
    ...
    <div className="flex flex-col w-full margin-25">
      ...
      {/* button add manager */}
       ...
       <button className="button-standart" onClick={handleOpenModelAddCategory} >
           Ajouter un nouveaux ingrediant
          </button> 
              ...
              .map((row) => {
               ...
               <TableCell   
                ...
                >
                   ...
                   <MdDelete  className="icon"  onClick={()=>deleteIngrediants(row._id)}/> 
                    </div>
